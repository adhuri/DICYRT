...
var initial={
 "countfood" : 0
   ...
   collection.group(keys,condition,initial,function (obj, prev) {         if (obj.food != null) if (obj.food instanceof Array) prev.countfood += obj.food.length;         else prev.countfood++;     }),(function (err, result) {
    if (err) {
      console.log(err);
    } else if (result.length) {
      console.log('Found:', result);
